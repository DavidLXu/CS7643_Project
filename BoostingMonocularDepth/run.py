...
def run(dataset, option):
    ...
    for image_ind, images in enumerate(dataset):
        ...
        img = addGaussNoise(img,var=0.001)
...
def addGaussNoise(s, var):
    var = 0.001 #random.uniform(0.0001, 0.04)
    #origin = skimage.io.imread(s)
    noisy = skimage.util.random_noise(s, mode='gaussian', var=var)
    return noisy


def addSaltNoise(s, var):#
    var = random.uniform(0.01, 0.2)
    #origin = skimage.io.imread(s)
    noisy = skimage.util.random_noise(s, mode='s&p', amount=var)
    return noisy


def addSpeckleNoise(s, var):
    var = random.uniform(0.0001, 0.04)
    #origin = skimage.io.imread(s)
    noisy = skimage.util.random_noise(s, mode='speckle', var=var)
    return noisy
